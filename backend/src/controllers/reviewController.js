...
try {
  ...
  if (lines.length > 198) {
    const trimmedLines = lines.slice(0, 197);
    fs.writeFileSync(filePath, trimmedLines.join('\n'), 'utf8');
    console.log('✅ Removed all lines after line 198');
  }
  ...
  try {
    ...
    if (fs.existsSync(gitPath)) {
      ...
      if (!gitignoreContent.includes(relativePath)) {
        ...
        console.log('✅ Added reviewController.js to .gitignore');
      }
      
      console.log('✅ Removed reviewController.js from git tracking');
    }
  } catch (gitError) {
